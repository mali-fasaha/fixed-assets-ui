...
import io.github.assets.uaa.config.SecurityBeanOverrideConfiguration;
...
public class FixedAssetsUaaKafkaResourceIT {
    ...
    @Autowired
    private FixedAssetsUaaKafkaProducer producer;

    @Autowired
    private FixedAssetsUaaKafkaConsumer consumer;
    ...
    @BeforeEach
    public void setup() {
        FixedAssetsUaaKafkaResource kafkaResource = new FixedAssetsUaaKafkaResource(producer);
    ...
    public void producedMessageHasBeenConsumed() throws Exception {
        restMockMvc.perform(post("/api/fixed-assets-uaa-kafka/publish?message=test"))
